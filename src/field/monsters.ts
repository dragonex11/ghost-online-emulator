...
import { yorkMoveType } from "./mob_move_types.js";
import { yorkAttackType } from "./mob_attack_types.js";
...
export type Monster = {
  ...
  walkArmed: boolean;
  /** Fractional px leftover (unused for York-sized ticks; kept for chase carry). */
  stepCarry: number;
  /** York AttackType — non-zero enables post-hit State 3 counter-attack. */
  attackType: number;
  ...
  attackUntil: number;
  /** Last flight destination sent via status-2 (Game4 flyer path). */
  flyDestX: number;
  flyDestY: number;
};
...
/**
 * EN (unpatched) protocol:
 * - Client walks/flies from Speed + Direction; dense Absolute SetPosition kills clips.
 * - Ground: York 1Hz Dest step + one MON_INFO per tick (keeps loot/touch synced).
...
const AI_TICK_MS = 16;
/** York/legacy `$MON_WALK_MS = 1000` — one Dest step per second. */
...
/** Thai Dest = 40*Speed px per walk tick (legacy `$Dest = Int(40 * Speed)`). */
...
/**
 * Where the EN client likely shows the mob (last Absolute + Speed prediction).
...
/** York MoveType 3 = flyer. Thai client flies them; Game4 needs server X patrol. */
...
/** York MobFactory.AttackType */
export function attackTypeFor(template: number): number {
  return yorkAttackType(template);
}

/** York MobFactory.MoveType: 0=stationary, 1=ground walk, 3=fly. */
export function moveTypeFor(template: number): number {
  return yorkMoveType(template);
}

/**
 * MoveType written to the client. Must match York for flyers (3): sending 1 made
 * Game4 ground-snap Y every frame while we pushed air Y → vertical twitching.
 */
...
function effectiveSpeed(template: number, rawSpeed: number): number {
  ...
  if (mt === 0) return 0;
  // Honor authored .prj/DB speed (~1.0). legacy floored to 2.6 — too fast vs official EN.
...
export async function loadMonsters(): Promise<void> {
  ...
  const rows = await query<RowDataPacket[]>(
    "SELECT monsterid, map, region, monsterX, monsterY, m_monsterid, health, speed, side FROM monster ORDER BY map, region, m_monsterid",
  );
  for (const r of rows) {
...
function writeMonSlot(buf: Buffer, s: number, m: Monster, enterSafe = false): void {
  // Server AI uses York MoveType; client MoveType may differ for Game4 anim (see clientMoveType).
  const yorkMt = enterSafe ? 0 : moveTypeFor(m.template);
  const moveType = enterSafe ? 0 : clientMoveType(m.template);
  const state = enterSafe ? 0 : yorkMt === 0 ? 0 : 1;
  const spd = enterSafe || yorkMt === 0 ? 0 : packetSpeed(m, 1);
...
/** Empty MON_ALL — used before staggered MON_REGEN (avoids Game4 crash on bulk spawn) */
...
/** Stagger MON_REGEN per mob — stable on Game4 vs one large MON_ALL at enter */
 ...
 * Bulk MON_ALL during enter crashes Game4; sending after stagger is stable.
 */
...
export function buildMonInfo(
  ...
  // York/legacy: PositionX/Y ALWAYS server m.x/m.y. HitX/HitY are spark only (+64/+66).
  // Status 2 (flyers): Game4 skips SetPosition and uses +20/+22 as flight destination.
  const px = m.x;
  const py = m.y;
...
/**
 * Ground MON_INFO: EN Absolute SetPosition restarts walk → idle flash.
...
/** Resume / arm flyer without Absolute status-1 (that freezes fly clips on EN). */
...
/**
 * Ground: York 1Hz Dest walk; MON_INFO only on facing / rare drift / combat.
...
/** Full reset on enter: revive + snap to exact .prj spawn (stable Game4 enter). */
